...
async def gen_message(bot: Bot, arcade_id: str) -> str:
    ...
    if arcade["last_action"] is not None:
        ...
        messages.append(
            f"\t于{time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(arcade["last_action"]["time"]))}（UTC+8）由{arcade["last_action"]["action"]["before"]}人{action2str(arcade["last_action"]["action"]["type"])}{arcade["last_action"]["action"]["num"]}人"
        )
