...
async def _(event: GroupMessageEvent):
    ...
    if balance == "inf":
        ...
        await query.finish(f"你现在有{balance}颗★哦~{reply(balance)}", at_sender=True)
    elif balance < 0:
        reply = rng.choice(replies_posi)
        await query.finish(
            f"你还欠迪拉熊{balance}颗★mai！{reply(balance)}", at_sender=True
        )

    await query.send("你还没有★哦~", at_sender=True)
